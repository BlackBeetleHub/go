...
func main() {
	...
	pkg := packageName(bin)
	repo := "github.com/BlackBeetleHub/go"
...
func build(pkg, dest, version, buildOS, buildArch string) {
	...
	timeFlag := fmt.Sprintf("-X github.com/BlackBeetleHub/go/support/app.buildTime=%s", buildTime)
	versionFlag := fmt.Sprintf("-X github.com/BlackBeetleHub/go/support/app.version=%s", version)
